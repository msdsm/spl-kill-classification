...
def train_preprocess(raw_videos_path): # "raw_video"を与える
    ...
    data_kill_path = './dummy_data/dummy_kill'
    data_notkill_path = './dummy_data/dummy_notkill'
    ...
    for mp4_list_name in notlabel_video_list:
        ...
        for unit_video_name in sorted(os.listdir(mp4_list_name_path)):
            ...
            if input_label[batch_size_idx][0] == 1:
                # killへ移動
                # ./dummy_data/dummy_notlabel/"mp4name"/"4秒動画名"/ -> ./dummy_data/dummy_kill/"mp4name"_"4秒動画"
                unit_video_path = os.path.join(mp4_list_name_path, unit_video_name)
                new_path = os.path.join(data_kill_path, mp4_list_name+unit_video_name)
                shutil.move(unit_video_path, new_path)
            else :
                # notkillへ移動
                # ./dummy_data/dummy_notlabel/"mp4name"/"4秒動画名"/ -> ./dummy_data/dummy_notkill/"mp4name"_"4秒動画"
                unit_video_path = os.path.join(mp4_list_name_path, unit_video_name)
                new_path = os.path.join(data_notkill_path, mp4_list_name+unit_video_name)
                shutil.move(unit_video_path, new_path)
    ...
    for mp4_file in os.listdir(raw_videos_path):
        ...
        shutil.move(mp4_path, "./dummy_trained_raw_video")
